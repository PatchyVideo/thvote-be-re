...
> **状态**：部分实施 —— ①服务端 Origin/Referer 校验已合并部署，但默认关闭（`REQUIRE_BROWSER_ORIGIN=false`），待灰度开启；②端口收口（`:18000` 直连关闭）仍是待办事项（阻塞于前端 codegen 直连 :18000，需先改走 nginx 代理）。详见 [BACKLOG B-048](../../BACKLOG.md)。
