...
角色/音乐的投票对象列表当前 bundle 在前端 `@touhou-vote/shared/data/{character,music}`。需求要求后端管理投票对象(初始化、去重合并、分类查询、详情)。本块把**角色/音乐**也迁到后端管理 + 前端从后端拉(作品已在 Block 2 做成此模式,本块推广)。
...
| 分类查询 | 仿 Block 2 `/vote-objects/works`:角色按首登作品、音乐按专辑 |
...
| `GET` | `/vote-objects/{category}/{id}` | 单个投票对象详情(character/music/work) |

返回形状对齐 Block 2 的 `/vote-objects/works`:`{ vote_year, groups:[{group, items:[...]}] }`。
> 三类(character/music/work)统一为同一套 `/vote-objects/*` 端点族,前端复用同一数据加载封装。
...
- 候选管理(schema 驱动)已支持 character/music/work 的导入/编辑;本块加合并字段 + 合并端点 + 分类查询。
- `/vote-objects/works`(Block 2)是同族端点的样板。
...
- `/vote-objects/works`(Block 2)先行;本块加 characters/music 同族端点。
