...
> **归档状态**：已归档 —— 2026-05-30 的模块级迁移进度快照，**内容已过期**（例：表中 `vote_data ✅` 一行所指的模块已于 2026-07-17 作为死代码删除；`查询结果 ✅ 9 端点` 所指实现已于 2026-07-18 由 B-050 计票重写取代）。
> 迁出时经代码复核**仍成立**的两条缺口已迁入 BACKLOG：**B-062**（`search_cps()` 恒空）、**B-063**（scraper 无测试）。
> 模块级现状请改看 [BACKLOG.md](../BACKLOG.md) 与 [CHANGELOG.md](../CHANGELOG.md)。归档于 2026-08-31。
> 创建日期：2026-05-12
