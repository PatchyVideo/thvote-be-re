...
**append-only 提示(2026-08-29,B-050-后补2 Step 2)**:`paper_answer` 现为 append-only 批次存储(迁移 0017 新增 `attempt` 批次号),本脚本按"首次提交"写入,`attempt` 恒为 1;重复灌注前的 wipe(按 `vote_id LIKE 'mock-%'` 前缀清理)不受影响——wipe 直接删行,与批次号无关。
