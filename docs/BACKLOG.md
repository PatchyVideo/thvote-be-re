...
> 创建日期：2026-04-27
> 最后更新：2026-08-13（#26 合入 main 盘点：**B-050-后补6 完成**——白名单已切 DB voteable、迁移 0016 已部署;B-021 盘点发现早已迁 V2 补记完成;B-048① 已随合并上线(默认关);另:08-13 测试环境事故已修复,见 CHANGELOG）
...
| **B-020** | mypy 接入 CI（现状:lint job 只 pip install 了 mypy 但从未运行）,先清告警再当门禁 | 半天-1 天（看现存告警量） |
