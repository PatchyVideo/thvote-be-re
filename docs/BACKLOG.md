...
| **B-050-后补2** | trend 投票演进:需先把 `raw_*` 改造成 **append-only** 存储(当前 `SubmitDAO._upsert` 是"删旧插新"覆盖式,见 `src/apps/submit/dao.py`,改票会丢历史行);现有 `trend`/`trend_first` 字段只是按"最新提交时间"分桶的近似值,非真实演进曲线。**Step 1 ✅(近似,2026-08-29)**——`compute_paper_results` 已产出按提交行 `created_at` 分桶的近似 trend,`queryQuestionnaireTrend` 已接通(含 DSL 子集路径),不再恒为空;`trendFirst` 恒空(问卷无本命概念)。**Step 2 进行中**——真实净增量需先落地 append-only 存储,`queryCharacterTrend`/`queryMusicTrend` 同样是近似值,仍待此项 | 中(需先改存储层) | 🟡 Step 2 需先定 append-only 方案 | 同上设计稿 §9.4 |
