...
| **B-050** | 计票重写：把 B-049 的 `invalidated` 软标记、人工复核结论等信号真正接入排名/计票逻辑（当前 `compute_dao` 仍只读未改动的旧计票路径，B-049 的处置动作对排名零影响） | ⏳ 待办（依赖 B-049 已提供的数据落点，尚未设计具体计票规则） | 🟡 需先定计票规则再动手 | 见 CHANGELOG `[2026-07-17] B-049 安全监控管理端 API` 条目「Record-only 边界」 |
