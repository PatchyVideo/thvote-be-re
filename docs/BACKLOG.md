...
| **B-050** ⚡ | **计票系统大重写（潜伏严重 bug）**：`ComputeDAO` 读的是**死表** `character/music/cp/questionnaire`(路径 B,无写入方、永远空;真实投票在 `raw_*` 路径 A)——**当前触发 compute-results 产出空/零排名**;且计票**完全不读 `user.removed`** → 封号对排名毫无影响。需把计票改读 `raw_*`(JOIN user 排除 `removed`,并预留 `invalidated` 作废票排除,配合 B-049 后台处置)+ 候选匹配/问卷结构对齐 + 联调。删死表 vote_data 已单独完成(2026-07-17);四个死模型待本项一并清理 | 🔴 高（上线前必修,否则真计票为空） | 🟢 可独立启动（大工程,需重写+联调） | 勘探结论见本次会话;`compute_dao.py`/`compute_service.py` |
