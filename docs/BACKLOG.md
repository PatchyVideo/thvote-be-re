...
> 创建日期：2026-04-27
> 最后更新：2026-07-17（新增 B-049 管理端安全监控台：后端 Plan 1 已实现；B-050 计票重写标为待办）
...
| **B-049** | 管理端安全监控台：`require_admin` 统一闸门(secret 常量时间比较 + IP 白名单,fail-closed) + 监控 API(概览/IP·设备聚类/可疑打分/投票浏览器/账号钻取) + 处置动作(单条投票作废/恢复、账号人工复核，仅记录) | 🟡 **后端 Plan 1 已实现** (2026-07-17)：migration `0014`(`raw_*.invalidated` + 索引 + `voter_review` 表) + `src/apps/admin/monitor/`(只读查询 + 处置端点，401 passed，flake8 clean)。**处置动作仅写标记/行，不接入排名计算**——排名效果推迟到 B-050。待：①`main.py` 的 `/admin/reload-config`、`/admin/discover*` 三个 ops 端点仍未纳入 `require_admin` 闸门(需先分析内部调用方，B-042 遗留)；②前端(Plan 2，Vue 管理台)未开始 | 🟢 前端(Plan 2)可独立开工 | [design](./superpowers/specs/2026-07-17-admin-console-vue-security-monitoring-design.md) / [backend plan](./superpowers/plans/2026-07-17-admin-console-security-monitoring-backend.md) |
| **B-050** ⚡ | **计票系统大重写（潜伏严重 bug）**：`ComputeDAO` 读的是**死表** `character/music/cp/questionnaire`(路径 B,无写入方、永远空;真实票在 `raw_*` 路径 A)——**当前 compute-results 产出空/零排名**;且计票**完全不读 `user.removed`** → 封号对排名零影响。需把计票改读 `raw_*`(JOIN user 排除 `removed`;并接入 B-049 后台已落地的 `raw_*.invalidated` 作废标记与 `voter_review` 复核结论)+ 候选匹配/问卷结构对齐 + 联调。删死表 vote_data 已完成(2026-07-17);四个死模型(`character/music/cp/questionnaire`)待本项一并清理 | 🔴 高（上线前必修,否则真计票为空） | 🟡 需先定计票规则再动手(大工程,重写+联调) | `compute_dao.py`/`compute_service.py`;数据落点见 B-049;CHANGELOG `[2026-07-17] B-049 安全监控管理端 API`「Record-only 边界」 |
...
| **B-042** | 测试环境配置硬化（2026-07-14 联调准备中实测发现）：① Nacos(`thvote_be`) **未配 `ADMIN_SECRET`**,而 `_check_admin_secret` 对未配置直接放行——:18000 公网可达,admin 全部端点(含问卷替换导入、候选合并)当前**无鉴权裸奔**;② scraper **Pixiv 凭据未配**(`Pixiv authentication failed`),二创提名刮削 Pixiv 源不可用。测试期可容忍,**上线前两项必须配置**(配完需重启容器,B-017) | 中 | 🟢 配置项,随时可做 | Nacos 控制台见 `docs/operations/nacos-config-center.md` |
