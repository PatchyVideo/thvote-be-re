...
| **B-064** | 迁移 0018 回填把「管理员封禁」当成「自助注销」:`WHERE removed = FALSE` 把 `ban_user` 封禁的账号也排除,导致其邮箱/手机被释放(可改头换面重新注册)、解封后无身份可登录。旧 schema 里自助注销与封禁不可区分,**需先拍板口径**(GDPR 抹除优先 → 维持现状;封禁可执行优先 → 全量回填并交给 `resolve` 拦截)。真 PG 已复现;仅影响"拿 0018 之前的存量库重放迁移",现有环境不受影响 | 低 | 🟡 等口径拍板 | [design §七](./superpowers/specs/2026-09-06-user-identity-model-design.md) |
...
| **B-037** | 安全块：二创提名校验(域名/发布时间/udid去重)+ 人工审核队列 + 提名时间窗 + 投票问卷门禁(弱校验) | 🟡 后端+管理端已完成 (2026-06-08)；前端待做（**前端=我方负责，准备开做**，2026-08-13） | — | 后端✅[plan](./archive/superpowers/plans/2026-06-08-security-backend.md)：配置/模型0007/纯校验/DAO/编排+门禁/GraphQL/管理端审核UI/公开查询;前端[plan](./superpowers/plans/2026-06-08-security-frontend.md)待实施 |
...
| **B-050-后补2** | trend 投票演进 | ✅ **已完成**(两步交付,2026-08-29):**Step 1**——`compute_paper_results` 已产出按提交行 `created_at` 分桶的近似 trend,`queryQuestionnaireTrend` 已接通(含 DSL 子集路径)。**Step 2**——`raw_*`/`paper_answer` 四表改造为 **append-only** 存储(迁移 0017,`paper_answer` 加 `attempt` 批次号 + 索引、删原 UNIQUE 约束;`SubmitDAO._upsert` 不再删旧插新),新纯函数 `src/apps/result/trend.py::net_delta_trends` 把角色/音乐/CP/问卷全类别 `trend`/`trend_first` 升级为真实净增量序列(改票产生负桶,不变量:每实体分桶净增量之和 == 最终票数),`compute_all` 与 DSL 子集重算路径均已接入。剩余小尾巴见 **B-061**(admin 监控列表展示 attempt 列) | — | 同上设计稿 §9.4;CHANGELOG 2026-08-29 两条目 |
| **B-050-后补3** | 上届对比:需历史 `final_ranking` 表有数据,且 `compute_dao.load_historical` 的 key 是 name、与 id-based 归票路径未对齐(现 `compute_service.py` 传空 `{}`)。**仍开放**——本轮契约层 `RankingEntry`/`CPRankingEntry` 的 8 个 `*_last_1`/`*_last_2` 历史字段已按 brief 要求固定填 -1 哨兵(见契约层 `_ranking_entry_from_dict` docstring 的"移除条件"),此项落地后需同步把契约层从硬编码 0 改成读 `rank[1]`/`rank[2]` 快照。**数据源已验证**(2026-08-22):老 Rust 生产 API `touhou.ai/vote-be/graphql` 的 `query*Ranking(voteYear:11)` 可直取上届完整榜(char totalVotes=13283 实测),字段与 `final_ranking` 表一一对应,无需找 dump;**导入动作等用户与协作方沟通后再执行** | 低 | ⏸ 等沟通(数据源就绪) | 同上设计稿 §9.4 |
| **B-053** | 通用"任意问卷题 × 投票结果"交叉分析 API + 前端（与 B-050-后补5 高级搜索同组，共用 `vote_id → {题code: [选项code]}` 索引，**该索引已在 2026-08-14 落地于 `src/apps/result/advanced_search/subset.py` 的 `build_facts`/`evaluate_subset`，可直接复用**）：本轮只落地了"性别"这一个特例分段(经配置指定的固定题)，通用查询(任选一道问卷题作为切分轴，交叉任意投票结果)与配套前端本轮未做——全题目预聚合会撑爆 Redis 榜单(244 角色 × 32 题 × 40 选项)，应按需算 | 低 | 🟢 可独立做(与 B-050-后补5 同组) | 本设计稿 §五「C1.2」/§九「BACKLOG」；子集原语见 `src/apps/result/advanced_search/subset.py` |
...
| **B-057** | voteable/work 重构收尾清单（2026-07-23 合入盘点发现）：①✅**已完成**（2026-08-22，`feat/b060-b057`）——`GET /admin/voteables`（联 work 出 workId/workName/workType，q/分页）+ `POST /admin/voteables/{id}` 编辑 work 归属（清 vote-objects 缓存），5 测试；admin-ui 消费页面可后补；② admin import 的 work 按 name 匹配/自动建 work（Task 5，计划内 tech-debt）；③ 上届 `final_ranking`(year=11) 导入未执行（会议结论要求"只导上届"）；④ 前端 Task 8/9（characterList/musicList 过滤逻辑、专辑名反查）+ **提交侧切 candidateId**（前置 B-050-后补6 已于 08-13 解除；**前端=我方负责**）；⑤ 设计稿/plan 的 checkbox 与状态维护（与 zfq 沟通） | 中 | 🟢 ①②③可独立；④前置已解除，入我方前端队列 | [work 统一设计稿](./superpowers/specs/2026-07-21-work-table-unified-design.md)；[实施计划](./superpowers/plans/2026-07-21-work-table-unified.md)；CHANGELOG 2026-07-23 条目 |
...
| **B-061** | admin 监控投票列表(`src/apps/admin/monitor/`)展示 `attempt` 列:B-050-后补2 Step 2 把 `raw_*`/`paper_answer` 改成 append-only 后,同一选民多次改票会在监控列表出现多条历史行,当前列表未展示批次号,管理员不易一眼分辨"哪几行属于同一次改票" | 低 | 🟢 可独立做 | `src/apps/admin/monitor/`;CHANGELOG 2026-08-29 append-only 条目 |
