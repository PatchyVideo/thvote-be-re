...
| **B-050** ⚡ | **计票系统大重写（潜伏严重 bug）**：`ComputeDAO` 读的是**死表** `character/music/cp/questionnaire`(路径 B,无写入方、永远空;真实票在 `raw_*` 路径 A)——**当前 compute-results 产出空/零排名**;且计票**完全不读 `user.removed`** → 封号对排名零影响。需把计票改读 `raw_*`(JOIN user 排除 `removed`;并接入 B-049 后台已落地的 `raw_*.invalidated` 作废标记与 `voter_review` 复核结论)+ 候选匹配/问卷结构对齐 + 联调。删死表 vote_data 已完成(2026-07-17);四个死模型(`character/music/cp/questionnaire`)待本项一并清理 | 🔴 高（上线前必修,否则真计票为空） | 🟡 **设计已就绪(2026-07-18,决策锁定),可写实现计划**(大工程,重写+联调) | **[设计稿:纯 id 记票重写](./superpowers/specs/2026-07-18-result-recount-id-based-design.md)**(id 贯穿/白名单/CP 无序/merged_into 黑掉/离线批量);`compute_dao.py`/`compute_service.py`;数据落点见 B-049 |
