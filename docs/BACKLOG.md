...
| **B-043** | 注册防刷：发验证码前强制人机验证（阿里云验证码 2.0,service 层双入口收口,默认 fail-closed）+ 补齐发码端点 per-IP 限流与短信 per-号码守卫（当前发码端点**无任何后端限流**,2026-07-16 勘探发现） | 🟡 后端壳已完成 (2026-07-17,`ENABLED=false` 零行为变化)；待 M0 控制台操作→Nacos 六键→拨 true;前端 widget 与发码限流待做 | — | [design](./superpowers/specs/2026-07-16-captcha-anti-abuse-design.md) / [接入手册](./operations/captcha-onboarding.md) |
