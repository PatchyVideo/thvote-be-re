...
| **B-043** | 注册防刷：发验证码前强制人机验证（阿里云验证码 2.0,service 层双入口收口,默认 fail-closed）+ 补齐发码端点 per-IP 限流与短信 per-号码守卫（当前发码端点**无任何后端限流**,2026-07-16 勘探发现） | 高（投票公正性） | 🟡 待 M0:开通服务/建场景/RAM AK 进 Nacos（需账号管理员操作） | [design](./superpowers/specs/2026-07-16-captcha-anti-abuse-design.md) |
